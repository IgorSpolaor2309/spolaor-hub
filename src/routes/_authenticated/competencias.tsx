...
type Situacao = "sem_atividade" | "com_atrasos" | "aguardando_cliente" | "pronta_revisao" | "em_andamento";
...
export function computeProgress(r: OverviewRow): { percent: number; applicable: string[] } {
  const modules: { key: string; weight: number; value: number | null }[] = [
    ...
    },
    {
      ...
      weight: 20,
      value: (() => {
        if (r.sol_total <= 0) return null;
        return Math.max(0, Math.min(1, r.sol_concluidas / r.sol_total));
      })(),
    },
    {
      key: "Guias",
      weight: 20,
      value: (() => {
        if (r.guias_total <= 0) return null;
        return Math.max(0, Math.min(1, r.guias_com_comprovante / r.guias_total));
      })(),
    },
    {
      key: "Pendências",
...
export function computeSituacao(r: OverviewRow): Situacao {
  const nada =
    r.checklist_total === 0 &&
  ...
  const comAtraso =
    r.pend_vencidas > 0 || r.guias_vencidas > 0 || r.proc_atrasados > 0;
