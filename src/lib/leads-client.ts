...
export const safeTrackLead = async (params: { data: any }) => {
  try {
    ...
    if (result?.prospectId) {
      window.sessionStorage.setItem('digital_sc_lead_id', result.prospectId);
    }
    
    return result;
  } catch (error) {
    console.warn("Lead tracking failed but continuing flow:", error);
    const storedId = typeof window !== 'undefined' ? window.sessionStorage.getItem('digital_sc_lead_id') : null;
...
export const getStoredProspectId = () => {
  ...
  return window.sessionStorage.getItem('digital_sc_lead_id');
};
