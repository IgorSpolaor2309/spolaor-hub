...
-- =========================================================
-- Processes: server-side paginated list RPCs
-- =========================================================
-- SECURITY INVOKER: RLS de company_processes / company_process_steps aplica.
-- Retornam JSONB { rows: [...], total: N } para permitir contagem única.
...
CREATE OR REPLACE FUNCTION public.list_company_processes_paginated(
  ...
  _prazo text DEFAULT NULL,      -- vencido | hoje | em_breve | sem_prazo
  _tab text DEFAULT 'todos',     -- todos | meus | aguardando | atrasados | concluidos
  _sort_by text DEFAULT 'prazo', -- prazo | empresa | responsavel | status | progresso | abertura
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE
      ...
      AND (
        _tab <> 'meus' OR (v_uid IS NOT NULL AND cp.responsavel_id = v_uid)
      )
      AND (
        _tab <> 'aguardando' OR cp.status IN ('aguardando_cliente','aguardando_orgao')
      )
      AND (
        _tab <> 'atrasados' OR (
      ...
      AND (
        _tab <> 'concluidos' OR cp.status = 'concluido'
      )
      AND (
        _prazo IS NULL OR _prazo = 'all'
        OR (
...
-- ---------------------------------------------------------
-- Etapas atribuídas ao usuário logado (Meus processos)
-- ---------------------------------------------------------
CREATE OR REPLACE FUNCTION public.list_my_process_steps_paginated(
  _search text DEFAULT NULL,
  _status_group text DEFAULT 'open', -- open | done | all
...
BEGIN
  IF v_uid IS NULL THEN
    RETURN jsonb_build_object('rows','[]'::jsonb,'total',0,'page',1,'page_size',_page_size);
  END IF;
  ...
  WITH base AS (
    ...
    WHERE s.responsavel_id = v_uid
      ...
      AND (
        _prazo IS NULL OR _prazo = 'all' OR (
  ...
  paged AS (
    SELECT * FROM base
    ORDER BY prazo ASC NULLS LAST, ordem ASC
    LIMIT v_limit OFFSET v_offset
  )
